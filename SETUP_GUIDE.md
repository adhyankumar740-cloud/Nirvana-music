...
- **Abhi ke code mein iska real use nahi hai:** `firebase-ai` dependency add hai lekin code mein kahin call nahi ho rahi — Jam room ka chat abhi locally simulate hota hai (canned responses), Gemini API abhi wire-up nahi hui. Toh filhaal key na ho to bhi app chalega, bas future AI features ke liye ready rakha gaya hai.
...
## 5. Workflow kya karta hai (summary)

- Push / PR / manual "Run workflow" pe trigger hota hai.
- JDK 17 + Gradle 9.1.0 setup karta hai (project ka wrapper nahi hai, isliye Gradle directly install hota hai).
